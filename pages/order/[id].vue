...
<template>
  <ModalBase v-model="orderCancelModal" title="주문취소">
    <div class="min-w-[500px] flex flex-col gap-10 my-8">
  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex items-center gap-1 p-1 bg-gray-200 rounded-md">
        <UButton color="white" class="h-9 px-3" @click="orderCancelModal = true">주문취소</UButton>
        <UButton color="white" class="h-9 px-3" @click="visible = true">결제독촉</UButton>
        <UButton color="white" class="h-9 px-3">수거지연</UButton>
        <UButton color="white" class="h-9 px-3">배송지연</UButton>
        <UButton color="white" class="h-9 px-3">메세지전송</UButton>
      </div>
    </div>
      ...
      <div class="flex items-start justify-between">
        <div class="flex items-center gap-5">
          <div class="flex flex-col gap-2">
            <span class="text-gray-500 text-xs">작업태그</span>
            <div class="flex items-center gap-2">
              <span class="text-xs h-7 bg-gray-200 px-3 rounded-full flex-center" v-if="true">정상주문</span>
          ...
          <div class="h-10 w-px bg-gray-200"></div>
          <div class="flex flex-col gap-2 ">
            <span class="text-gray-500 text-xs">고객주문상태</span>
            <span class="font-semibold text-green-600">세탁중</span>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <USelect v-model="workAdd" :options="['수거','검수','출고','배송']" class="min-w-[100px]" size="md" placeholder="작업추가"/>
      ...
      <div class="flex flex-col gap-2 mt-10">
        <div class="flex items-center">
          <a v-for="item, index in ['주문','수거','검수','세탁','결제','출고','배송','완료']" @click="active = index"
            ...
            <span :class="[active === index ? 'font-semibold text-blue-600' : 'text-gray-500']" class="relative z-10">{{ item }}</span>
        ...
        <div class="flex items-center">
          <div 
            class="flex items-center gap-1 flex-1 bg-blue-50 rounded-md p-1" v-for="item, index in 8" :key="item"
            :class="[active === index ? 'block' : 'invisible']"
          >
            <UButton class="h-7 flex-1 flex-center text-xs rounded" color="white" >삭제</UButton>
            <UButton class="h-7 flex-1 flex-center text-xs rounded">완료</UButton>
          </div>
        </div>
      </div>
    </div>
    ...
    <div>
      <div class="h-11 px-1 gap-1 bg-gray-900 rounded-md flex items-center mb-4">
        <a 
          v-for="item in ['주문정보','검수상품','결제내역','작업내역']"
